...
func (n *Notifier) send(sub *Subscription, data stdjson.RawMessage) error {
...
func newClientSubscription(c *Client, namespace string, channel reflect.Value) *ClientSubscription {
	sub := &ClientSubscription{
		...
		channel:     channel,
		in:          make(chan stdjson.RawMessage),
...
func (sub *ClientSubscription) deliver(result stdjson.RawMessage) (ok bool) {
	select {
	case sub.in <- result:
		return true
...
func (sub *ClientSubscription) forward() (unsubscribeServer bool, err error) {
	...
	for {
		...
		case 1: // <-sub.in
			val, err := sub.unmarshal(recv.Interface().(stdjson.RawMessage))
			if err != nil {
				return true, err
			}
...
func (sub *ClientSubscription) unmarshal(result stdjson.RawMessage) (interface{}, error) {
